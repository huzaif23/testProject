...
import com.example.mangotech2.a123ngo.Rates;
...
public class GetFareDetails extends AsyncTask<String, Void, String> {
    Rates rates;
    String Error_Message="";
    public GetFareDetails(Rates rates){
        this.rates=rates;
    }
    ...
    protected String doInBackground(String... strings) {
        ...
        try{
            URL url = new URL(DocumentEnums.ApiUrl+"/api/Booking/GetFareDetails");

          //  URL url = new URL("http://123ngo.somee.com/api/Account/ChangePassword");

            String CityDetailId=strings[0];
            String VehicleTypeId=strings[1];

            JSONObject postDataParams = new JSONObject();
            postDataParams.put("VehicleTypeId", VehicleTypeId);
            postDataParams.put("CityDetailId", CityDetailId);
    ...
    protected void onPostExecute(String result) {
        ...
        if(Error_Message!=""){
            rates.showresponse(Error_Message);
            Error_Message="";
        }else{
        ...
        try {
            ...
            if(Error_Message.isEmpty()||Error_Message.toString()==""){
                ...
                String base_price = vals.get("base_price").toString();
                String base_price_later = vals.get("base_price_later").toString();
                String unit_price = vals.get("unit_price").toString();
                String unit_price_later = vals.get("unit_price_later").toString();
                String unit_time_fare = vals.get("unit_time_fare").toString();
                String unit_time_fare_later = vals.get("unit_time_fare_later").toString();

                rates.txtnowstartingprice.setText(base_price);
                rates.txtlaterstartingprice.setText(base_price_later);
                rates.txtnowminimumprice.setText(unit_price);
                rates.txtlaterminimumprice.setText(unit_price_later);
                rates.txtmovingperkmprice.setText(unit_time_fare);
                rates.txtwaitingperhourprice.setText(unit_time_fare_later);
            }else {
                rates.showresponse(Error_Message);
            }
